...
const detail = (kind: string, name: string, owners: { kind: string; name: string }[] = []): ResourceDetail => ({ summary: { kind, name, namespace: "payments", ready: "0/1", health: "degraded", labels: {}, age_seconds: 60 }, yaml: "", owner_refs: owners } as ResourceDetail);
...
beforeEach(() => {
  ...
  vi.mocked(k8s.listEventsFor).mockImplementation(async (_ns, kind, name) => [{ ts: "2026-09-05T23:59:00Z", type_: "Warning", reason: "BackOff", message: `${kind} warning`, involved_kind: kind, involved_name: name, count: 5 }, { ts: null, type_: "Warning", reason: "Unrelated", message: "other resource", involved_kind: "Service", involved_name: name, count: 1 }, { ts: null, type_: "Normal", reason: "Pulled", message: "image already present", involved_kind: kind, involved_name: name, count: 1 }]);
});
it("connects failing container, exact events, observed owners and scoped export", async () => {
  ...
  expect(report).toHaveTextContent("UID correlation unavailable");
...
it("redacts credential-like owner event messages before display and export", async () => {
  vi.mocked(k8s.listEventsFor).mockImplementation(async (_ns, kind, name) => [{ ts: null, type_: "Warning", reason: "Failed", message: "token: super-secret-owner-event", involved_kind: kind, involved_name: name, count: 1 }]);
  renderInvestigation();
