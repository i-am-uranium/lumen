...
describe("log evidence", () => {
  ...
  it("rejects replacement across capture", async () => { const getResource = vi.fn().mockResolvedValueOnce(detail("uid-1")).mockResolvedValueOnce(detail("uid-2")); const result = await captureIncidentLogEvidence(request, { getResource, capture: vi.fn().mockResolvedValue({ text: "old logs", truncated: false, bytes: 8 }), now: () => new Date(0) }); expect(result.status).toBe("replaced"); expect(result.text).toBe(""); });
  it("bounds and redacts oversized captures", async () => { const raw = `Authorization: Bearer synthetic-secret\n${"x".repeat(25_000)}`; const result = await captureIncidentLogEvidence(request, { getResource: vi.fn().mockResolvedValue(detail("uid-1")), capture: vi.fn().mockResolvedValue({ text: raw, truncated: true, bytes: 20_000 }), now: () => new Date(0) }); expect(result.truncated).toBe(true); expect(result.text.length).toBeLessThanOrEqual(20_000); expect(result.text).not.toContain("synthetic-secret"); });
